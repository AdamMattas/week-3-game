...
var hangman = {
	...
	words : ['dilithium', 'shuttlecraft', 'defiant', 'voyager', 'vulcan', 'cardassian', 'bajoran', 'nebula', 'galaxy', 'humanoid', 'tricorder', 'betazoid', 'neutron', 'wormhole', 'andorian', 'subspace', 'positronic', 'holodeck', 'disruptor', 'shields', 'data', 'enterprise', 'assimilated', 'ferengi', 'collective', 'federation', 'starfleet', 'transporter', 'phasers', 'android', 'klingon', 'romulan', 'orbit'],
	letterQuery : [''],
	guessed : '',
	placeholders : '',
	wordToGuess : '',
	...
	newGame: function(){
		...
		hangman.wordToGuess = hangman.words[Math.floor(Math.random()*hangman.words.length)];
		...
		for(var i = 0; i < hangman.wordLength; i++){
			...
			if(hangman.wordToGuess.charAt(i) == letter.toLowerCase()){
				...
				if(hangman.correctGuesses == 1){
					hangman.stopVid();
					hangman.playVid6();
				}else if(hangman.correctGuesses == 4 && hangman.correctGuesses != hangman.wordLength){
					hangman.stopVid();
					hangman.playVid7();
				}else if(hangman.correctGuesses == 6 && hangman.correctGuesses != hangman.wordLength){
					hangman.stopVid();
					hangman.playVid8();
				}
				// check to see if game has been won and plays success video
					...
					document.getElementById("over-success").innerHTML = hangman.wordToGuess;
